...
def process_directory(input_dir, output_dir, scale_factor=2, apply_sharpen=False, sharpen_factor=2.0, progress_callback=None):
    ...
    image_files = [f for f in os.listdir(input_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.tiff', '.bmp', '.gif'))]
...
class UpscaleApp:
    ...
    def start_processing(self):
        ...
        try:
            ...
            self.progress_bar["maximum"] = len([f for f in os.listdir(input_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.tiff', '.bmp', '.gif'))])
